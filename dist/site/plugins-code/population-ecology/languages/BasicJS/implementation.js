const createExports = (sendMessage) => {
    const runSimulation = (initialSize, dayDuration, getTick, totalDays, initMessage) => {
    ...
    return Promise.resolve({
         ...
         */
        createSimulationExponential: (initialSize, growthRate, days = 10, dayDuration = 3000) => {
            if (initialSize <= 0)
                throw new Error("initialSize must be positive");
            if (days <= 0)
                throw new Error("days must be positive");
            if (dayDuration < 500)
                throw new Error("dayDuration must be at least 500ms");
            runSimulation(initialSize, dayDuration, (day) => Math.round(initialSize * Math.exp(growthRate * day)), days, { type: "addPopulation", initialSize, growthRate, dayDuration, label: "Exponential" });
        },
        /**
         ...
         */
        createSimulationCustom: (populations, dayDuration = 3000) => {
            ...
            runSimulation(initialSize, dayDuration, (day) => populations[day], totalDays, { type: "addCustomPopulation", populations, dayDuration });
        },
    });
};
