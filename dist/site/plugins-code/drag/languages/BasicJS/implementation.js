const createExports = (sendMessage) => {
    return Promise.resolve({
        runSimulation: (getNextPosition, stepSize) => {
            ...
            const getDrag = (velocity) => {
                ...
                const dragAccelerationMagnitude = (0.5 * RHO * A * CD * relativeSpeed ** 2) / M;
                return {
                    x: -dragAccelerationMagnitude *
                        (relativeVelocityX / relativeSpeed),
                    y: -dragAccelerationMagnitude *
            ...
            let currentAnimation = "predicted";
            const getNextPositionActual = (currentPos, currentVel, dt) => {
                ...
                if (!isComplete) {
                    ...
                    while (currentFrameSimulatedTime < FRAME_SIMULATED_TIME_ADVANCE) {
                        if (currentAnimation === "predicted") {
                            ...
                            const nextPredicted = getNextPosition(currentPositionPredicted, currentVelocityPredicted, SIMULATION_TIMESTEP, getDrag);
                            predictedPath.push({
                                x: nextPredicted.x,
                                y: nextPredicted.y,
                            });
                            ...
                            // Actual path
                            const actualResult = getNextPositionActual(currentPositionActual, currentVelocityActual, ACTUAL_TIMESTEP);
