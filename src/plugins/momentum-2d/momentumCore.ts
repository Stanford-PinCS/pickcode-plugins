...
export type MomentumInputOverride = Partial<Omit<MomentumInputs, "bodies">> & {
...
const normalizeBody = (
  ...
  const radius = clamp(sanitizeNumber(body?.radius, fallback.radius), 16, 70);
  const safeX = clamp(
...
export const normalizeInputs = (
  ...
  const width = clamp(
    sanitizeNumber(inputs?.width, DEFAULT_INPUTS.width),
    520,
    1500
  );
  const height = clamp(
...
> | null => {
  ...
  const canStick =
    Math.abs(requiredTangentialImpulse) <= maxFrictionImpulse + EPSILON;
  const frictionMode =
    Math.abs(relativeSpeedAlongTangent) <= EPSILON
      ? "none"
      : canStick
  ...
  const tangentialImpulseMagnitude =
    frictionMode !== "sliding"
      ...
      : -Math.sign(relativeSpeedAlongTangent) *
        Math.min(maxFrictionImpulse, Math.abs(requiredTangentialImpulse));
...
const solveCollision = (inputs: MomentumInputs): CollisionSolution => {
  ...
  const collisionPositionA = add(
    bodyA.position,
    scale(bodyA.velocity, collisionTime)
  );
  const collisionPositionB = add(
    bodyB.position,
    scale(bodyB.velocity, collisionTime)
  );
  const separationAtCollision = subtract(
    collisionPositionB,
    collisionPositionA
  );
...
export const buildReport = (model?: MomentumModel): MomentumReport => {
  ...
  const [solverCollisionPositionA, solverCollisionPositionB] =
    solution.collisionPositions;
  ...
  const collisionPositionB = solverCollisionPositionB
    ...
    : null;
  const bodyADerived = buildBodyDerived(
    bodyA,
    finalVelocityA,
    collisionPositionA
  );
  const bodyBDerived = buildBodyDerived(
    bodyB,
    finalVelocityB,
    collisionPositionB
  );
