...
async function main() {
  ...
  const pluginName = await prompt(
    "Enter plugin name (kebab-case, e.g., my-plugin): "
  );
  ...
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(pluginName)) {
    console.error(
      "Error: Plugin name must be in kebab-case (lowercase letters, numbers, and hyphens only)"
    );
    process.exit(1);
  }
  ...
  if (existsSync(pluginDir)) {
    console.error(
      `Error: Plugin "${pluginName}" already exists at ${pluginDir}`
    );
    process.exit(1);
  }
  ...
  console.log(`\nNext steps:`);
  console.log(
    `  1. Edit src/plugins/${pluginName}/Component.tsx to implement your plugin UI`
  );
  console.log(
    `  2. Edit src/plugins/${pluginName}/state.ts to define your plugin state`
  );
  console.log(
    `  3. Edit src/plugins/${pluginName}/messages.ts to define message types`
  );
  console.log(
    `  4. Edit src/plugins/${pluginName}/languages/BasicJS/implementation.ts to implement runtime logic`
  );
}
